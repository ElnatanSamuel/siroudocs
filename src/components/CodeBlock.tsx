      ...
      {(filename || !terminal) && (
        <div className="code-header">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-1 text-white">{filename || language}</span>
          </div>
          <button
            onClick={handleCopy}
            className="flex items-center gap-1 text-white transition-opacity"
          >
            {copied ? (
          ...
          {lines.map((line, i) => {
            ...
            return (
              <div key={i} className="flex">
                {!terminal && (
                  <span className="token-line-number select-none inline-block w-8 text-white text-right mr-4 shrink-0">
                    {i + 1}
                  </span>
                )}
                {terminal && (
